...
type CaseSet<'a when 'a : comparison> private (theMap : Map<int, 'a>) =
    ...
    override x.Equals other =
        theMap = (other :?> 'a CaseSet).InnerMap

    override x.GetHashCode () = theMap.GetHashCode ()
    ...
    interface ICollection<'a> with
        ...
        member x.Add _ = raise (NotSupportedException "ReadOnlyCollection")
        member x.Remove _ = raise (NotSupportedException "ReadOnlyCollection")
